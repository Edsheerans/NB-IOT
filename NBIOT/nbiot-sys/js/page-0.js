$(function () {
    ...
    $.ajax({
        ...
        type: "get",
        dataType: "json",
        url: "http://loradata.handsometechs.com/api/notifyids",
...
function bindGridData(data) {
    ...
    for (i = 0; i < data.length; i++) {
        ...
        data_grid[i]["Lat"] = data[i][2];
        data_grid[i]["Long"] = data[i][3];
        data_grid[i]["Att"] = data[i][4];
...
function BaiduMap(data) {
    var id = data[0];
    var mapparam = {
        "coords": data[3] + "," + data[2],
        "from": 1,
        "to": 5,
        "ak": "7v9p7Ey6dkGiTMO4cAG9oFFBbxKEEzKd"
    };
