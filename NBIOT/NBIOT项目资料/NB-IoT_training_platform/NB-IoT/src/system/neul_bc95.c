...
int neul_bc95_set_band(int band)
{
    ...
    memset(neul_dev.rbuf, 0, 6);
    ret = neul_dev.ops->dev_read(neul_dev.rbuf, 6, 0, 200);
    if (ret <= 0)
    {
        //read bc95 read set return value info failed
        return -3;
...
int neul_bc95_active_network(void)
{
    ...
    memset(neul_dev.rbuf, 0, 30);
    ret = neul_dev.ops->dev_read(neul_dev.rbuf, 30, 0, 200);
    if (ret <= 0)
    {
        //read bc95 read set return value info failed
        debug("time out.\n");
    ...
    memset(neul_dev.rbuf, 0, 30);
    ret = neul_dev.ops->dev_read(neul_dev.rbuf, 20, 0, 5000);
    if (ret <= 0)
    {
        //read bc95 read set return value info failed
        debug("time out.\n");
    ...
    memset(neul_dev.rbuf, 0, 30);
    ret = neul_dev.ops->dev_read(neul_dev.rbuf, 25, 0, 200);
    if (ret <= 0)
    {
        //read bc95 read set return value info failed
        debug("time out.\n");
...
int neul_bc95_reboot(void)
{
    ...
    memset(neul_dev.rbuf, 0, 60);
    ret = neul_dev.ops->dev_read(neul_dev.rbuf, 60, 0, 6000);
    if (ret <= 0)
    {
        debug("time out.\n");
        //read bc95 read set return value info failed
...
int neul_bc95_create_udpsocket(unsigned short localport)
{
    ...
    memset(neul_dev.rbuf, 0, 30);
    ret = neul_dev.ops->dev_read(neul_dev.rbuf, 20, 0, 2000);
    if (ret <= 0)
    {
        //read bc95 read set return value info failed
        return -1;
...
int neul_bc95_set_auto_connect(int flag)
{
    ...
    debug("AT+NCONFIG=AUTOCONNECT,TRUE...\n");
    if (flag)
    {
        //set auto connect
...
int neul_bc95_prepare_transmit(void)
{
    ...
    memset(neul_dev.rbuf, 0, 6);
    ret = neul_dev.ops->dev_write(p1, strlen(p1), 0);
    ret = neul_dev.ops->dev_read(neul_dev.rbuf, 6, 0, 500);
    if(ret <= 0)
    {
        debug("time out.\n");
    }
...
int uart_data_read(char *buf, int maxrlen, int mode, int timeout)
{
    ...
    while(1)
    {
        ret = UartGetBuffer(&Uart1, (uint8_t *)buf, maxrlen, &read_cnt);
        buf += read_cnt;
        retlen += read_cnt;
        // read enough
        if(ret == 0 && retlen >= maxrlen)
        {
            return retlen;
        }
        // time out
...
int uart_data_write(char *buf, int writelen, int mode)
{
    ...
    UartPutBuffer(&Uart1, (uint8_t *)buf, writelen);
    return 0;
}
